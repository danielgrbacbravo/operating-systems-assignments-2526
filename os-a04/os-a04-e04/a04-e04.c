...
static void enqueue_frame(int queue[], int number_of_frames, int *queue_front,
                          int *queue_size, int frame_index) {
  int back_index = (*queue_front + *queue_size) % number_of_frames;
  queue[back_index] = frame_index;
  (*queue_size)++;
}

static int dequeue_frame(int queue[], int number_of_frames, int *queue_front,
                         int *queue_size) {
  int frame_index = queue[*queue_front];
  *queue_front = (*queue_front + 1) % number_of_frames;
  (*queue_size)--;
  return frame_index;
}

static int choose_clock_victim_frame(Frame frames[], int queue[],
                                     int number_of_frames, int *queue_front,
                                     int *queue_size, int *writes_to_swap) {
  int scan_count;

  for (scan_count = 0; scan_count < number_of_frames; scan_count++) {
    int frame_index =
        dequeue_frame(queue, number_of_frames, queue_front, queue_size);

    if (!frames[frame_index].is_referenced && !frames[frame_index].is_dirty) {
      return frame_index;
    }

    if (frames[frame_index].is_referenced) {
      frames[frame_index].is_referenced = 0;
    }

    enqueue_frame(queue, number_of_frames, queue_front, queue_size,
                  frame_index);
  }

  {
    int victim_frame_index =
        dequeue_frame(queue, number_of_frames, queue_front, queue_size);
    if (frames[victim_frame_index].is_dirty) {
      (*writes_to_swap)++;
    }
    return victim_frame_index;
  }
...
int main(void) {
  ...
  int *clock_queue;
  int clock_queue_front = 0;
  int clock_queue_size = 0;
  ...
  clock_queue = (int *)malloc((size_t)number_of_frames * sizeof(int));

  if (all_accesses == NULL || frames == NULL ||
      page_faults_per_process == NULL || clock_queue == NULL) {
    free(all_accesses);
    free(frames);
  ...
  for (step_index = 0; step_index < number_of_accesses; step_index++) {
    ...
    if (frame_index != -1) {
      status_text = "HIT";
      frames[frame_index].is_referenced = 1;
      if (access_type == 'W') {
      ...
      if (destination_frame_index == -1) {
        destination_frame_index = choose_clock_victim_frame(
            frames, clock_queue, number_of_frames, &clock_queue_front,
            &clock_queue_size, &writes_to_swap);
      }
      ...
      frames[destination_frame_index].is_referenced = 1;

      enqueue_frame(clock_queue, number_of_frames, &clock_queue_front,
                    &clock_queue_size, destination_frame_index);
    }
