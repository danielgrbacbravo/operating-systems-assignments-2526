/*
 * main.c, Test harness for Exercise 1: RAID 0
 *
 * Input format (stdin):
 *   Line 1: DATA_SIZE  (integer, bytes)
 *   Next DATA_SIZE bytes: raw data (may contain newlines)
 *   Next line: FAIL_DISK (-1 = none, 0-3 = disk to kill before read)
 */
...
int main(void)
{
    ...
    if (scanf("%d", &fail_disk) != 1) fail_disk = -1;
        ...
        disk_reset();
    int rc = store_file_raid0(data, data_size);
    ...
    rc = read_file_raid0(buf, data_size);
